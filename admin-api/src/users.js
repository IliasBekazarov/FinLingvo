...
export async function fetchMe(id) {
  ...
  if (u.state.hearts !== before) await persist(); else persist();
  return publicView(u);
}
...
// reward params come from server config below.
export async function completeLesson(id, { lessonId, mistakes, isReview }) {
  await load();
  const u = cache.users.find(x => x.id === id);
  if (!u) throw new Error('not found');
  const st = u.state;
  ...
  // First-time clear: full rewards. Review: smaller rewards, no streak bump, no heart loss.
  let xpGain = 0, gemGain = 0;
  if (!isReview) {
    // Lose hearts equal to mistakes (but not below 0). The per-question loss is
    // handled by the client UX; server truth happens here on completion.
    const heartsLost = Math.min(st.hearts || 0, mistakes || 0);
    st.hearts = Math.max(0, (st.hearts || 0) - heartsLost);
    if (st.hearts < MAX_HEARTS && heartsLost > 0) {
      // Restart refill timer when first heart is lost.
      st.heartsRefilledAt = Date.now();
    }
    xpGain = 10 * (lessonQuestionCount(lessonId) || 5) / 5; // base correlation; client also adds 10/correct
    // Use simpler: +50 base + 25 perfect bonus
    xpGain = 50 + (perfect ? 25 : 0);
    gemGain = 20 + (perfect ? 5 : 0);
    if (!wasCompleted) st.completedLessons.push(lessonId);
    if (perfect && !st.perfectLessons.includes(lessonId)) st.perfectLessons.push(lessonId);
    bumpStreak(st);
  } else {
    // Practice: small XP, no gems, no hearts lost.
    xpGain = perfect ? 10 : 5;
    gemGain = perfect ? 5 : 0;
  }
  ...
  u.lastSeenAt = new Date().toISOString();
  await persist();
  return { user: publicView(u), reward: { xp: xpGain, gems: gemGain, perfect, isReview } };
}

function lessonQuestionCount() { return null; } // unused stub
